...
<div class="post_box">
    ...
    If (getAvatar($db, $User['username']) != Null) {
         ?><a href="./forums/memberlist.php?mode=viewprofile&u=<?php echo $User['user_id'];?>" target="_blank"><img src="<?php Echo getAvatar($db, $User['username']);?>" alt="Avatar"/></a><?php
    }
    ?>
...
?>
    ...
    Else {
        ...
        While (isset($Posts[$X])) {
            ...
            If (getAvatar($db, $User['user_id']) != Null) {
                $Avatar = getAvatar($db, $User['user_id']);
                Echo "<div class=\"gravatar\">";
                Echo "<a href=\"./forums/memberlist.php&mode=viewprofile&u={$User['user_id']}\" target=\"_blank\"><img src=\"./forums/download/file.php?avatar={$Avatar}\" width=\"50\" height=\"50\"></a>";
                Echo "</div>";
            }
    ...
    <div id="comment_form">
        ...
        <form action="./blog/view/<?php echo $PostID;?>/" method="POST">
            ...
            <input type="submit" name="comment_submit" value="Submit" class="submit_btn" style="float: right;" />
        </form>
    </div>
    <?php
    }
